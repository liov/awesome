...
After
option go_package = "github.com/hopeio/gox/proto/openapiconfig";
...
# go.info.runtime.firstmoduledata: relocation target go.info.github.com/hopeio/gox/reflect.moduledata not defined
...
## minikube 有秘钥 /etc/kubernetes/pki/etcd
...
# cors 跨域
测试的时候,跨域请求一直报错
`Access to XMLHttpRequest at 'http://localhost:8090/api/moment?pageNo=1&pageSize=10' from origin 'http://localhost' has been blocked by CORS policy: No 'Access-Control-Allow-Origin' header is present on the requested resource.`
...
# RollupError: xxx "default" is not exported by "node_modules/.pnpm/dayjs@1.11.13/node_modules/dayjs/dayjs.min.js", imported by "xxx".
...
工具链不匹配,解决办法 `rustup default nightly-gnu`s

# IDEA git 巨卡
安装git的时候使用自带的openssh,别用windows的
反正重装选回自带的就不卡了
...
那些ts文件在build目录中,换个目录名可以了
不知道为啥,改了tsconfig配置没生效,有缓存?
build被git ignore了?

# gcc编译undefined reference to `current_timestamp'
原因是include目录下同时拥有动静态库,gcc先去找了.a文件
只有dll文件能编译成功,只有.a文件一样的错
-Bdynamic 和 -Bstatic 参数指定

# pnpm10.5.2 link --global有问题

# ERR_PNPM_VIRTUAL_STORE_DIR_MAX_LENGTH_DIFF 
pnpm i -g 

# pnpm link --global
pnpm link --global 将当前项目作为全局依赖项
pnpm link --global @hoperio/utils 想要链接的命令不能用了，不能加--global了,会认为是当前包想要以@hoperio/utils的名字做全局依赖
pnpm link @hoperio/utils 可以用

# 无线调试连不上
手动adb连接
adb pair 192.168.5.7:42649
Enter pairing code: xxxx
adb connect 192.168.5.7:35705 

# go private 仓库
git config --global github.com.oauth-token xxxxx_your_token_xxxxx
GOPRIVATE="github.com/xxx"
git config --global url."https://$GIT_USER:$GIT_TOKEN@$GOPRIVATE".insteadOf "https://$GOPRIVATE"

# iptables failed: iptables --wait -t nat -A DOCKER -p tcp -d 0/0 --dport 8428 -j DNAT --to-destination 172.17.0.2:8428 ! -i docker0: iptables: No chain/target/match by that name
docker run --name vmetrics -p 8428:8428 -d -v `pwd`/victoria-metrics-data:/victoria-metrics-data \
victoriametrics/victoria-metrics:latest --selfScrapeInterval=5s -storageDataPath=victoria-metrics-data
30bc8f0f55b77b6a26b7ecaf570ab1e95c6586b7320c4774641bab4af856176d
docker: Error response from daemon: driver failed programming external connectivity on endpoint vmetrics (082cf901dd72802c839f8bf03a9084e859c1a339e5b7ef3e44e02ff189e37807):  (iptables failed: iptables --wait -t nat -A DOCKER -p tcp -d 0/0 --dport 8428 -j DNAT --to-destination 172.17.0.2:8428 ! -i docker0: iptables: No chain/target/match by that name.
(exit status 1)).
系统使用了 nftables 而不是传统 iptables
